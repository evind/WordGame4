...
class Play_Screen(Play_ScreenTemplate):
    ...
    def finished_button_click(self, **event_args):
        ...
        if len(self.answer_box.text) < 1:
            ...
            Globals.time = anvil.server.call(
                "calculate_time", self.item["curr_time"], self.item["end_time"]
            )
            Globals.user_input_list = self.answer_box.text.split()
            Globals.rules = anvil.server.call(
                "evaluate_answer", Globals.source_word, Globals.user_input_list
            )

            # Get user-agent info, then log their attempt to the log table.
            # Note: this server.call format looks strange, but it's how black
            # wanted it.
            user_agent = anvil.http.request(
                anvil.server.get_api_origin() + "/get-user-agent", json=True
            )["user-agent"]
            anvil.server.call(
                "log_attempt",
                Globals.rules,
